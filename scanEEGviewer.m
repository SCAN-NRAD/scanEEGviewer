...
function scanEEGviewer(varargin)
	...
	if(exist('fileName', 'var'))
		...
		if(nargin > 1)
			...
			eventFilename = strsplit(fileName, '.');
			eventFilename = [eventFilename{1:end-1} '-events*'];
		end
	else
		% if data is provided
...
% 	channelLat = zeros(numel(chLabel), 1);
	...
	function changeEEGsigDisp()
		...
		labelHyph = cellfun(@(x) strfind(x, '-'), dispChLabel, 'uni',false);
		hyphFlag = ~any(cellfun(@isempty, labelHyph)) && ~any(diff(cell2mat(labelHyph)));
		if(hyphFlag)
			labelChar = cellfun(@(x) x(1:labelHyph{1}-1), dispChLabel, 'uni',false);
		end
		
		labelundL = cellfun(@(x) strfind(x, '_'), dispChLabel, 'uni',false);
		undLFlag = ~any(cellfun(@isempty, labelundL)) && ~any(diff(cell2mat(labelundL)));
		if(undLFlag)
			labelChar = cellfun(@(x) x(1:labelundL{1}-1), dispChLabel, 'uni',false);
		end
		
		if(~hyphFlag && ~undLFlag)
			labelVal = cellfun(@(x) double(lower(x)), dispChLabel, 'uni',false);
			labelChar = cellfun(@(x) x(x > 57), labelVal, 'uni',false);
		end

		labelChar = cellfun(@sum, labelChar);
		[elUni, ~, elGroup] = unique(labelChar, 'stable');
		
% 		labelIsChar = cellfun(@(x) ~isempty(x), labelChar);
% 		chanRight = cellfun(@(x) strcmp(char(x(end)), 'r') || ...
% 								strcmp(char(x(end-1:end)), 're') || ...
% 								strcmp(char(x(end-1:end)), 'ri'), labelChar(labelIsChar));
% 		chanLeft = cellfun(@(x) strcmp(char(x(end)), 'l') || ...
% 								strcmp(char(x(end-1:end)), 'li') || ...
% 								strcmp(char(x(end-1:end)), 'le'), labelChar(labelIsChar));
% 		channelLat = zeros(numel(dispChan), 1);
% 		channelLat(chanRight) = 1;
% 		channelLat(chanLeft) = 2;
...
% 			channelLat = channelLat(biChIdx);
	...
	function plotEEGsignals()
		...
		% plot channels in left / right / unknown groups
% 		if(any(channelLat == 0))
% 			plot(axEEG, selDataIdx/fs, tmpEEGdata(:, channelLat == 0), ...
% 				'Color',[63 63 63]/255, 'HitTest','off');		% unknown
% 		end
% 		if(any(channelLat == 1))
% 			plot(axEEG, selDataIdx/fs, tmpEEGdata(:, channelLat == 1), ...
% 				'Color',[207 109 60]/255, 'HitTest','off');		% right
% 		end
% 		if(any(channelLat == 2))
% 			plot(axEEG, selDataIdx/fs, tmpEEGdata(:, channelLat == 2), ...
% 				'Color',[84 129 230]/255, 'HitTest','off');		% left
% 		end
	...
	function eventWin(evIdx, evStart, evEnd, selCh)
		...
		if(isempty(raterID))
			...
			eventFilename = strsplit(fileName, '.');
			eventFilename = [eventFilename{1:end-1} '-events-' raterID];
